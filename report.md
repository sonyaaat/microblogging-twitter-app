...
- **AI Tools Used in This Report:** GitHub Copilot (code generation), Claude (prompt refining)

---

## 1. App Description

The app is a microblogging web application, similar to Twitter. Users can register and log in, create short text posts (up to 280 characters), view a global feed of all posts sorted by newest first, like and unlike posts, reply to posts (one level deep) and view user profiles. There is no follower system, no private messaging and no reposting. 
...
| Treat LLM output as draft | Week 4 / Arora et al. | Reviewed every file Copilot generated before moving to the next prompt | Caught the missing `await params` bug early instead of finding it in 5 files at once |
| Ban vague words unless quantified | Week 4 / ChatGPT-derived | Wrote "280 characters max" instead of "short posts", "return 401" instead of "handle auth" | Copilot actually respected the limits instead of guessing |
...
| Specify testing goal and scope | Week 9 / Augusto et al. | Test prompts said exactly what to test, what framework to use, and what NOT to test | Tests did not try to test NextAuth internals or unrelated things |
...
| Structured Copilot instruction file | Week 10 / GitHub Blog | Created `.github/copilot-instructions.md` with rules for security, logging, pagination | Did not have to repeat the same rules in every single prompt |
| Restrict to standard logging library | Week 12 / Rodriguez et al. | Every prompt said "use only Winston from lib/logger.ts, no console.log anywhere" | No console.log in the codebase, all logs go through the same place |
...
| 4 | Code structure | Whether the project has a clear folder structure with separation between api, components, lib and types. | Good structure makes it easier to find files, add features and understand the codebase. |
| 5 | Code readability and consistency | Whether functions are easy to understand, API routes follow the same pattern and the code style is consistent across files. | Inconsistent code is hard to review and maintain. Structured prompts should produce more uniform output. |
...
| Total development time | ~3 hours | ~4,5 hours | Week 2 was faster to start but broke often. Week 13 took longer upfront because of structured prompts and reviews but had almost no big surprises at the end. |
...
| Code structure |Good structure, most files separated, but some files were too long amd it was possible to peparate them more | Clear separation: app/api, components, lib, types folders with one responsibility per file. There aren't so many files with lots of lines of code. | Week 13 prompts explicitly required a file structure upfront which Copilot then followed. |
